...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, in_planes, planes, args, device, stride=1, only_fc=True, option="B"
    ):
    ...
    def forward(self, x):
        ...
        if self.stride != 1 or self.in_planes != self.planes:
            ...
            active_downsample = downsample.weight * self.tasks_masks[self.task_id][
                -1
            ].to(self.device)
            ...
            downsample = F.conv2d(
                x,
                weight=active_downsample,
                bias=None,
                stride=downsample.stride,
                padding=downsample.padding,
                groups=downsample.groups,
            )
            ...
            out += downsample * (
                ...
                .reshape((-1, 1, 1))
                .expand((downsample.size(-3), downsample.size(-2), downsample.size(-1)))
                .to(self.device)
            )
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, args, device, only_fc=True, task_id=0):
        ...
        self.conv1 = nn.Conv2d(
            3, _outputs[0], kernel_size=7, stride=2, padding=3, bias=False
        )
            ...
            if not self.only_fc:
                ...
                for layer in range(len(self.masks_union[1])):
                    for block in range(0, len(self.masks_union[1][layer])):
                        for conv in range(2):
                            self.masks_union[1][layer][block][
                                conv
                            ] = 1 * torch.logical_or(
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            basicblock = block(
                self.in_planes, planes, self.args, self.device, stride, self.only_fc
            )
    ...
    def forward(self, x):
        ...
        active_weight = self.fc.weight * self.tasks_masks[self.task_id][-1][0].to(
            self.device
        )
        active_bias = self.fc.bias * self.tasks_masks[self.task_id][-1][1].to(
            self.device
        )
        ...
        for task_id in range(self.num_tasks):
            ...
            for layer in range(len(self.num_blocks)):
                for block in range(self.num_blocks[layer]):
                    ...
                    name = "layer{}.{}.downsample.mask.task{}".format(
            ...
            masks_database["fc.weight.mask.task{}".format(task_id)] = self.tasks_masks[
                task_id
            ][-1][0]
            masks_database["fc.bias.mask.task{}".format(task_id)] = self.tasks_masks[
                task_id
            ][-1][1]
    ...
    def _load_masks(self, file_name="net_masks.pt", num_tasks=1):
        ...
        for task_id in range(num_tasks):
            ...
            self.tasks_masks[task_id][-1][0] = masks_database[
                "fc.weight.mask.task{}".format(task_id)
            ]
            self.tasks_masks[task_id][-1][1] = masks_database[
                "fc.bias.mask.task{}".format(task_id)
            ]
        ...
        if name == "conv1.weight" and not net.only_fc:
            ...
            for layer_num in range(len(net.num_blocks)):
                for block_num in range(net.num_blocks[layer_num]):
                    ...
                    elif name == "layer{}.{}.downsample.0.weight".format(
                        layer_num + 1, block_num
                    ):
                        param.data = old_param.data * (
                            1 - net.trainable_mask[1][layer_num][block_num][-1]
    ...
    with torch.no_grad():
        ...
        for X, y_true in data_loader:
            ...
            correct_preds += correct_preds_batch(
                y_preds.cpu()[:, offset_a:offset_b], y_true.cpu() - offset_a, topk=topk
            )
...
def training_loop(
    ...
    for epoch in range(0, epochs):
        ...
        model, optimizer, train_loss = train_resnet(
            train_loader, model, criterion, optimizer, old_params, device, task_id
        )
            ...
            if "bns" not in name:
                ...
                for task_id in range(num_tasks):
                    if "bns.{}.".format(task_id) in name:
                        ...
                        state_dict[name] = pretrained_model.state_dict()[
                            pretrained_name
                        ]
        else:
...
def resnet_conv_block_pruning(
    ...
    if layer_num == 0:
        ...
        if conv_num >= 0:
            ...
            name_bn = "layer{}.{}.downsample.1.bns.{}".format(
            ...
            if layer_num == 0:
                ...
                if conv_num == 1:
                    if (
                        ...
                        downsample_name_bn = "layer{}.{}.downsample.1.bns.{}".format(
                            layer_num, block_num, task_id
                        )
    ...
    for it in range(1, args.num_iters + 1):
        ...
        if "lenet" in args.model_name:
            if task_id == 0:
                ...
                init_params = "{}_task{}_{}classes_{}_{}_it{}_seed{}.pth".format(
                    args.model_name,
                    task_id + 1,
...
def main():
    ...
    parser.add_argument(
        "--dataset_name", type=str, default="imagenet1000", help="dataset to use"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="fraction of importance to keep in fc layers",
    )
    ...
    parser.add_argument(
        "--num_iters", type=int, default=1, help="number of pruning iterations"
    )  # 3
    parser.add_argument(
        "--prune_batch_size",
        type=int,
    ...
    parser.add_argument(
        "--train_epochs", type=int, default=90, help="number training epochs"
    )  # 90
    parser.add_argument(
        "--retrain_epochs",
        type=int,
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-1, help="initial learning rate"
    )  # 1e-1 SGD; 1e-2 Adam
    parser.add_argument(
        "--decay_epochs_train",
        nargs="+",
    ...
    parser.add_argument(
        "--wd", type=float, default=1e-4, help="weight decay during retraining"
    )
    ...
    if TRAIN:
        for task_id in range(start_task, num_tasks):
            ...
            net = train(
                args, net, train_loader, test_loader, device=device, task_id=task_id
            )
    ...
    if EVAL:
        ...
        for task_id in range(NUM_LEARNED):
            ...
            print(
                "Task {} accuracy with task_id: ".format(task_id + 1),
                accs1[task_id],
                accs5[task_id],
            )
